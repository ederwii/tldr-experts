...
| `tldrx story reopen <id> --note "…"` | Give one build story another run of attempts. `--for-fix` opens a fix round on a story already `done` — one named defect, no attempt consumed, same DoD and same reviewer. `--as-is` settles a story you finished BY HAND from its branch as it stands: the DoD, the review and the merge run over the commits already there, and no developer is spawned. A red DoD blocks it, and so does a branch carrying nothing its epic has not already got. |
