...
| `tldrx story reopen <id> --note "…"` | Give one build story another run of attempts. `--for-fix` opens a fix round on a story already `done` — one named defect, no attempt consumed, same DoD and same reviewer. Either way, a developer that leaves the tree exactly as it was handed blocks the story before the DoD, with your note in the reason — nothing changed, so nothing could have landed it. `--as-is` settles a story you finished BY HAND from its branch as it stands: the DoD, the review and the merge run over the commits already there, and no developer is spawned. A red DoD blocks it, and so does a branch carrying nothing its epic has not already got. Reopening a `blocked` story also releases the dependents it alone was holding, each with its own signed record. |
...
| `tldrx expert list \| create \| train \| recompute \| rescore \| packs` | See [Experts](/guides/experts). `rescore` re-reads the knowledge files and derives their evidence again — free, after a change to what counts as evidence. `packs enable\|disable\|status` is the one switch for the stack packs — off by default. |
