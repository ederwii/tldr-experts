...
### 5. Or hand it all to the engine

The four steps above keep you at the keyboard. A small, well-seeded change can go the whole
way with nobody watching — measured once: a one-story bugfix went from seed to merged PR in
45 minutes with no human input:
...
`--gates none` lets every gate sign itself, `--questions none` lets the loop take each
question's own `Recommended:` pick, `--ship pr` opens the PR when the run is done. What each
flag means, how to write the seed, how to check on it and how to stop it:
