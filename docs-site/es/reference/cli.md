...
| `tldrx story reopen <id> --note "…"` | Le da a una story de Build otra tanda de intentos. `--for-fix` abre en cambio una ronda de arreglo sobre una story que ya está `done`: un defecto concreto, sin consumir intento, con el mismo DoD y el mismo revisor. En ambos casos, un developer que deja el árbol exactamente como lo recibió bloquea la story antes del DoD, con tu nota en el motivo: nada cambió, así que nada pudo haberlo resuelto. `--as-is` liquida una story que terminaste A MANO a partir de su rama tal como está: el DoD, la revisión y el merge corren sobre los commits que ya están ahí, y no se lanza ningún developer. Un DoD en rojo la bloquea, y también una rama que no lleve ningún commit que su epic no tenga ya. Reabrir una story `blocked` también libera a los dependientes que solo ella retenía, cada uno con su propio registro firmado. |
