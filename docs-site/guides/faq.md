...
tldrx story reopen S3 --as-is --note "…"   # you finished the branch by hand: no developer, same gates
...
smaller) of that command's combined output, one file per red check, written before the
story's worktree is removed. The story's own log, `04-build/log/<story>.md`, quotes the
...
did not keep the output, and their worktrees are gone: re-run the command yourself.
