...
- **A retry spends — unless the previous attempt already finished the work AND said nothing
  against it.** Ordinarily it is a fresh metered stage under the same phase ceiling and the same
  `--max-usd`. When the bound is spent the loop stops on the failure's own exit `5`, and the last
  line says the count — `3 consecutive stage failures at 03-plan/plan …` — so the `run.failed`
  payload on your phone says the loop tried, rather than a bare `5`. But a retry of ANY stage
  whose declared outputs are already complete and valid on disk settles at `$0.00` with no turn
  spawned at all, rather than paying to reproduce output that is already correct — ONLY when the
  previous attempt's own failure was external to the work: a `checks:` failure since corrected by
  hand (the turn itself succeeded), or the wall dying on it (`timeout`, `process_killed`,
  `rate_limit`), or a process death with nothing parseable at all. A `checks:` failure that would
  still fail against what is on disk is NOT skipped by this — it still buys a real turn, exactly
  as before. Neither is a failure the AGENT ITSELF reported: an unreadable or self-contradicting
  result, an empty result, a developer that asked a question and left the worktree untouched, or
  any death this repo could not classify — structurally valid files left behind are not proof the
  work is done when the agent that wrote them said it was not; that retry still buys a real turn
  too, named on the report line as `prior attempt reported <kind>; spawning`. The settled row
...
allowed. Since gh #271 that refusal does not block a story whose tree holds work — committed or
not, since the DoD runs before the facilitator's own commit anyway; git-ignored files and the
framework's own state dirs do not count: it is recorded (on the review log, on `task.done`,
in `## Unknowns`) and the facilitator's own Definition of Done decides — green goes on to
review, red blocks with both reasons. A refusal with no work at all — a tree the developer
...
equivalent, "`git stash` is not granted" for one without. A chained line with no work in the
...
**The developer died reading its own history — and a cure it had already been given.** Two
halves of the same thing, since gh #287 and gh #294. The allowance held no read verb at all: a
developer could write its worktree and not look at it, and one that ran
`git -C <worktree> log --oneline -5` was refused, re-spawned with the cure, refused again, and
took the story with it — for a command that changes nothing. `git status`, `git log`, `git diff`
and `git show` are granted now. **`-C <path>` is not, on purpose**: it points git at any
directory — the epic worktree, another story's worktree, your own checkout — so the cure is
"drop `-C <path>` and run git from your own worktree", never "ask for `-C`". The other half is
why a cure that says only WHAT does not stick: three consecutive developers on one story
appended `; echo "EXIT:$?"` to a DoD command to read the exit code, and "run each command alone"
answered a question none of them was asking — from their side the `echo` was not a second
command, it was how you read an exit code. So a chain that exists only to capture the outcome
(an `echo` naming `$?`, a redirect of the command's own output) is now told the thing that makes
it unnecessary: the facilitator re-runs the Definition of Done after the developer and records
each command's exit code, so the number the `echo` would print is written down whether the
developer captures it or not. That clause says only what is measured here and holds for every
provider — what an agent's own execution tool hands back to the model is the provider CLI's
business, and the prompt is shared. The developer prompt says the same, from the same constant.

**A story needs a command your workspace never declared.** Measured on a live unattended run: a
story whose acceptance criterion required the migration to be *generated by `dotnet ef`*, in a
workspace declaring `dotnet build` and `dotnet test`. Four developers died and two reviewer rounds
ran over ~3 h; the fourth developer wrote, in plain words, that `dotnet ef` was not grantable and
made no code change — and the run paid for another attempt anyway. A developer's grant is built from
your declared `commands:`, exactly and with trailing arguments, so a command that is not one of them
(or one of them plus arguments) cannot be run, and no re-run, reopen note or relaunch changes that.
Since gh #285 the FIRST refusal says so in the operator's own terms: where the recorded reason used
to stop at "a headless turn has nobody to approve it", it now names the cure — add a `commands:` slot
whose value is exactly `dotnet ef`, the tool and its subcommand, never the story's own arguments. It
is on the block reason, the review log and the handoff's `## Unknowns`, so the first dead developer
hands you the exact edit instead of the fourth. A `git` line is never blamed on `commands:`: the
developer's git allowance is its own list, and "add a slot" there would be a false cure.
