...
- **A watcher card can now say "there is nothing to query", and be believed (#212).** Measured
  ...
  stage failed and the cost was spent on the honest answer. `Query` was the one checked section with no absent form — every other section
  had `absent:` and it had nothing. It now accepts one line as well as a fenced block:
  ...
  second reader of that grammar, and a card has exactly one. `watch check`, `watch arm`'s
  post-merge screen and every other view print it as `unobservable — <reason>` with its source,
  through one renderer; the Watch handoff lists the card under an `**unobservable**` line rather
  than quietly omitting it. Prose under `## Query` is still refused, in the same words as before
