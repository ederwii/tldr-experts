...
async function build(entrypoints: readonly string[], outdir: string): Promise<void> {
  const result = await Bun.build({ entrypoints: [...entrypoints], outdir, target: "node" });
  if (!result.success) {
...
await build(hookSources, join(DIST, "hooks"));
