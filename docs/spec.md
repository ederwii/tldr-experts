...
| `tasks[].metered` | bool | n | **Additive.** `false` iff `cost_usd` is `null`; absent means metered. The two always travel together, and a `null` cost without it is a schema error. For a SPAWNED turn it is decided in one place (`spawnAgent.ts`'s `interpret`), from whether the provider's result document actually carried a USD figure — never from WHICH provider ran, which is what let a Claude turn read as metered whether or not a dollar figure ever came back. For an in-session turn it is whether a cost was DECLARED |
...
| `tasks[].banked_before_refusal` | `true` | n | **Additive.** This row was recorded AHEAD of a refusal on an unreadable `questions.md` — the turn ran, so its cost is in the ledger before `--commit` refuses on the way out. It is also the ONLY row a re-run's fingerprint may match against: the refusal leaves the stage `running` for the operator to fix the file and re-run, so this is the one case where the same `result.json` is expected back. Absent on every ordinary completed task, including one a gate reject sent back and a retry re-ran |
| `tasks[].dedupe` | str | n | **Additive.** WHY this row was recorded as its own rather than matched against an earlier `banked_before_refusal` row its cost and outputs resemble: `none — no session id`, because a result with no `session_id` cannot be told apart from a second, genuinely distinct unmetered turn. Absent when no such question arose. The alternative was to guess, and a guess here drops a turn out of the ledger |
...
3. **The Definition of Done, re-run by the facilitator** in that worktree, through the same runner `dod-gate` uses. All
   ...
   **A cached RED is narrower than a cached green (2026-09-06).** Each row additionally carries `command_hash` — the
