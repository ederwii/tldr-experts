...
| `attempts` | int 1..5 | n (2) | Developer attempts one unit of this stage's work gets before it blocks. **The phase ceiling is sized for this many** (§2.11, `planBudget`), so raising it raises what the phase holds rather than what one attempt may spend |
