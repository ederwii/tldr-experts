...
| `tldrx drive <--attended\|--unattended>` | nothing — no workspace, no run, no file | nothing. stdout is the session mandate for the driver of a run: the three-role protocol, evidence discipline, parking, review calibration by stakes and budget honesty, plus the gate and prepare/commit sections that differ between the two modes. Versioned with the package. A mode is required and never guessed | 0,1 |
