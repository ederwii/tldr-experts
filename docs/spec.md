...
the loop carries on; rejected, it stops and prints the note; lapsed, exit `4` with the same
lines, after one `gate.timeout`. It WAITS FOR a signature and never produces one: there is no
