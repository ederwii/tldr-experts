...
| `stages[].gate.and_continue` | `true` | n | **Additive.** WHAT THE REJECTION ASKED FOR, written by `tldrx reject --and-continue` alone (§5). `tldrx reject` covers two acts under one verb — "stop, I will look" and "redo it this way and carry on" — and this key is the second one said out loud, so an unattended `run auto --wait-gates` waiting in another process re-runs the stage instead of exiting `4`. Only ever `true`: absent means stop, which is what a bare rejection writes today and what every run.yml written before this key says. Written on every rejection (as the key or as nothing at all, never inherited from a previous one) and cleared when the stage parks on its gate again, so it never outlives the rejection it describes |
...
| `tasks[].cache_creation_input_tokens` / `.cache_read_input_tokens` | number ≥0 | n | **Additive.** The same turn's prompt-cache accounting, off the same `usage` object, and the reason a row can now explain its own `cost_usd`: a field-audited turn recorded `input_tokens: 84, output_tokens: 37150, cost_usd: 1.98` for a 124 KB prompt while the provider billed it 4,911,750 cache reads — 84 + 37,150 tokens do not cost $1.98 and nothing on the row could say what did. Gated SEPARATELY from the split above, and each one on its own evidence: a cache WRITE is billed at 1.25x an input token and a cache READ at 0.1x, nothing adds one to the other, and the two calls measured below show the shape that matters — the second reported `cache_creation: 0` beside 37,059 cache reads, so a both-or-nothing rule would drop exactly the number worth having. Written only when a counter came back strictly positive; absent means "no positive counter reached the ledger", never a zero standing in for one. Never folded into `input_tokens` or into `tokens`: three quantities, three prices |
...
| `tasks[].failure_kind` | `timeout`\|`rate_limit`\|`process_killed`\|`empty_result`\|`non_zero_exit`\|`result_error`\|`malformed_result`\|`unclassified`\|`asked_no_diff` | n | **Additive** (gh #348, the family of #341/#298). A named cause for a FAILED turn, derived from the same shape `.error`'s text already comes from (`spawnAgent.ts`'s `describeFailure`) — never a second, disagreeing account. `rate_limit` reuses gh #298's own detection (the last `rate_limit_event` frame the stream carried named a `status` other than `"allowed"`); it invents no new value for the status the wall itself reports, which remains unattested (#298). A NARROWER fallback (gh #341) also earns `rate_limit`: a turn with no such frame, that would otherwise fall to `unclassified`, whose raw stdout/stderr matches a small exported regex for the REPORTED (not measured — no raw capture of a turn that actually died against the wall exists in this repo) shape of a session-limit API error. It never overrides a more specific cause (`timeout`, `process_killed`, `malformed_result`, or a named `errors[]`/subtype disagreement) — only the residual bucket `describeFailure` could not otherwise explain. `result_error` (pre-merge review, 2026-09-15) is the exitCode-0 sibling of `non_zero_exit`: the result document itself reports `is_error: true` with a named reason while the PROCESS exited `0` — `non_zero_exit` is now STRICT to exitCode !== 0, so the label never contradicts the exit code it names (a turn shaped this way used to read `"claude exited 0 with is_error=true: …"` under `non_zero_exit`, an audit label lying about its own evidence, AGENTS.md §7). `unclassified` still carries the raw signal (exit code, `is_error`, any subtype) in `.error` — this key never loses information `.error` already had. `asked_no_diff` (gh #364) is the ONE value on this list `spawnAgent.ts` never derives: the Build executor's own classification of a developer turn that returned a perfectly readable envelope, named at least one thing in `questions_asked`, and left the worktree exactly as it was handed — only in a run whose `questions_policy` (§2.2) resolves the Build stage to anything but `human`. It requeues the story while `attempt < attempts` (the same bound a red DoD gets), rather than a terminal `blocked` only a human's `story reopen` could lift. Absent on every row written before this key, on every row that finished `ok` for a reason other than #364's, and on a read-cap kill, where `stopped_by` already names the death in full. Classification only: nothing here decides a retry, and no new retry budget was added |
| `tasks[].exit_disagreement` | str | n | **Additive** (gh #375). Written ONLY on a row whose `status` is `"done"` despite a process exit code that disagreed with the provider's own result document (`"exit <n>, subtype success"`) — settled that way because every output the stage declared was found on disk, non-empty, at the moment the verdict was taken (`exitDisagreement.ts`'s `settleExitDisagreement`, called once from each of the three headless spawn sites in `runNext.ts` rather than pasted three times). A turn that disagrees but is MISSING a declared output stays `"failed"` under its ordinary `failure_kind` exactly as before this existed, and never carries this key — the two are mutually exclusive on any one row. Measured on a live run (#375): a watch developer wrote its output and `claude` exited 1 while its own result document said `subtype: "success"`; before this key existed the row read `status: "failed"`, `failure_kind: "unclassified"`, and `run auto` spent one of five relaunches re-running work that was already done |
...
nothing; a `created_with`/`last_written_by`, when present, is a string; an `input_tokens`/`output_tokens`/`cache_creation_input_tokens`/`cache_read_input_tokens`, when present, is a finite number ≥ 0, a `banked_before_refusal` is `true`, a `dedupe` is a string and a `role`, when present, is a string — each absent-or-valid, so a `run.yml` written before any of them existed still loads; every `gates_policy` value is `human\|auto\|agent` and every key names a stage in the file; a `gate.evidence`, when present, is complete and its `role`/`verdict` are §2.17 values;
...
a `ship`, when present, carries all three of `push` (bool), `pr` (bool) and `auto_merge` (`never` \| `checks`) — a
...
| `attempts` | int 1..5 | n (2) | Developer attempts one unit of this stage's work gets before it blocks. **The phase ceiling is sized for this many** (§2.11, `planBudget`), so raising it raises what the phase holds rather than what one attempt may spend. Read by the executor, the `next` brake, `warn_at_pct`, `story reopen` and the dashboard's "attempt N of M" — and, since #214, by `budget show`, `budget raise`'s closing line, `run estimate` and the `budget-gate` hook, which price the remaining work with the cursor stage's own value (tolerant `buildStageDefaults`; an unreadable preset gives the shipped 2). Before, those four asked with the shipped 2, so an `attempts: 1` stage was reserved a developer turn and a reviewer nobody dispatches, and the hook could deny a `tldrx next` the brake allowed |
...
| `reviewer_share` | number 0..1 | n (0.25) | The reviewer's share of a story's price (§5, "The Build ceilings"). Since #333, resolved the same tolerant way as `attempts` above by `budget show`, `run estimate` and the `budget-gate` hook, matching the `next` brake |
| `story_cap_multiplier` | number 1..20 | n (3) | What `03-plan/budget.yml`'s price for a story is multiplied by to get its developer's ceiling (gh #277). The price is a planner's figure written before any code was read, so it is treated as an order of magnitude rather than a forecast; a value below 1 is **refused**, never clamped. Since #333, resolved the same tolerant way as `attempts` above by `budget show`, `run estimate` and the `budget-gate` hook — before, those three asked with the shipped 3 regardless of what the stage declared, quoting (or refusing on) up to several times the money the brake would actually price |
| `story_cap_floor_usd` | number 0..200 | n (4) | The least a priced story's developer may be given, whatever the multiplied price says — clamped to the stage's own ceiling. The developer-side sibling of `REVIEWER_FLOOR_USD`. Since #333, resolved the same tolerant way as `attempts` above by `budget show`, `run estimate` and the `budget-gate` hook |
...
**`append` dedupes on normalised text (gh #216).** A field audit measured 27 verbatim duplicates in one workspace's
`facts.yml` — one import that ran twice — inlined into every what/how/build prompt with no filter. Every writer above
goes through `FactsStore.append`, which now compares the incoming text, trimmed/whitespace-collapsed/case-folded
(`normaliseFactText`), against every LIVE fact's; a match returns the existing fact instead of minting a row. A
...
| `alternatives` | str[] | n | **Additive** (gh #251). The options this answer did NOT take, as `<letter>) <text>`, written only when non-empty — an empty list is refused by the validator, because on disk it would say the options were counted and there were none. Written by `run auto`'s own answer under `questions_policy: recommended` beside `decided_by: agent-default`, so a reader of the fact alone sees what a machine chose against; a person's `tldrx answer` writes no such key |
| `recommended_why` | str | n | **Additive** (gh #251). The block's `Recommended:` line's own reason, verbatim minus its `[src:]` token. Absent means the block carried no reason or a person answered — never that a reason was checked and found empty |
| `source.decided_by` | `owner\|driver\|agent-default` | n | **Additive.** WHO DECIDED, as against `who`, which is who typed it. `agent-default` (gh #251) is the THIRD value: `run auto` took the asker's own `Recommended:` option under `questions_policy: recommended` (§2.2) — a row that said `owner` there would lie in the dangerous direction, and a row that said nothing would hide that a machine decided. A fact is exactly the artefact that gets cited later, so a row that cannot say which of the two it was says nothing rather than implying the stronger one: absent means "not stated", never "owner". Written by `tldrx facts add --decided-by`, where the flag is REQUIRED, and by `tldrx answer --decided-by`, where it is OPTIONAL: that path is also driven by the `answer-capture` hook, which fires on an agent's own `Write` and on a person's edit alike (§4) and so cannot honestly say which of the two answered. The field stays optional so every row written before it existed keeps validating, and `tldrx answer` prints the absence and its reason on stdout rather than leaving a reader to notice it. Rendered in every `{{facts}}` block as `· decided by owner` / `· decided by driver`, appended only when present, so a fact without it reaches a prompt byte-identical to before |
...
| `irreversible:` / `money:` (optional) | Extra metadata keys (gh #251); `true` pins the question to a PERSON whatever the run's `questions_policy` (§2.2) says — `irreversible` because the answer cannot be walked back by a later `--supersede` (a dropped table, a sent message), `money` because the answer spends or changes what may be spent. ONE reader, `run auto`'s own answer path: a pinned block is escalated exactly as under `human`, even when it carries a `Recommended:` line. Every surface that LISTS or NOTIFIES a question ignores them — they say who answers, not whether the question is shown. Additive and tolerant like `advisory:`: absent means "not pinned", and only the literal `true` pins, so a typo'd value is never a silent opt-in to a machine deciding |
...
| `Recommended:` line (optional) | `Recommended: <letter> — <why> [src: …]`, after the options and before the `[Answer]:` slot. The ASKER's own call, carried onto the decision card and into the `question.raised` payload. Additive: a block written before this line existed parses exactly as it always did. Grammar is strict about the two machine-readable parts — one letter `A`–`E`, and a dash before any prose — precisely so the reader can be tolerant about the rest: the letter comes first, then any combination, in this order, of its `)`, a `— <why>` reason and a trailing `[src: …]` token or tokens, so `Recommended: B`, `Recommended: B)` and `Recommended: B [src: …]` all read as B (gh #323 — the reason used to be required before a citation, and that shape parked an unattended run). ONE reader, `parseRecommendedLine` (`src/core/text/questions.ts`): the loop's pick, the decision card and `tldrx seed check` all go through it, and the first line starting `Recommended:` is the block's line, readable or not; **a line that does not match is IGNORED, never refused**, and `tldrx questions lint` says nothing about it. The line is guidance, so a typo must cost the guidance and not the gate. When an `agent` gate's evidence note (§2.17) also carries a `recommend:` entry for the same id, **the note wins** |
...
asked_by asked_at`, and the only optional keys anything reads are `affects:`, `advisory:`, `irreversible:` and `money:` — there is no `default:`,
...
code that implements it. The one other thing that answers without a person is `run auto` under `questions_policy: recommended`
(§2.2, gh #251), and it is neither a timeout nor a first-option guess: it takes the option the block's OWN `Recommended:`
line names, at the moment the stage parks, records it as `decided_by: agent-default`, and tells the owner — a block with no
such line, or pinned with `irreversible:` / `money:`, still waits for a person. **Measured, and the reason this paragraph exists:** the driver of `260830-money-and-payments`
...
| a needle is given against the workspace's `.tldrx/memory/facts.yml` and it occurs ONLY inside a fact's `source: {who, when, run, q, decided_by}` map | `ok` | provenance is not content (gh #231). Measured 2026-09-14: `absent:.tldrx/memory/facts.yml#tenant-credits` was refused on `run: 260914-tenant-credits` — the run id of the facts `run auto` had just auto-answered. Deliberately narrow: only that one file, only a flow map whose EVERY key is one of those five (a hand-edited `source:` with any other key is searched in full), and never the rest of a fact — a needle in `fact:`, `area:`, `alternatives:` or `recommended_why:` still refuses, even on a fact this run wrote a moment ago |
...
**`run.relaunched` was added 2026-09-12 (gh #252).** It is `tldrx run auto --until-done [<n>]` running its own loop
AGAIN, in the same process, after an exit it could do nothing else with — a stage failure past `--retry-failed` (`5`),
a thrown error that used to reach the shell as a bare `1` with nothing on the ledger, a refusal with no money behind
...
bound), `last_line` (the attempt's last line — its head, an ellipsis and its TAIL when it is long, because the
tail is the end a person acts on and a front slice cuts it, the #235 shape) and `comparand` (what the NEXT attempt
is compared against: the refusal `next` named, and the last line only where it named none, gh #297); the envelope's `stage` is the
cursor stage at the moment of the relaunch, `cost_usd` is `0`. It is NEVER written over exit `4` (a person's), over
a `budget.blocked` or the loop's own `--max-usd` (nothing in-process moves a ceiling), or twice over the same
refusal — one derivation, `relaunchVerdict` in `core/facilitator/runAuto.ts`, decides both this event and whether the
run-end notification (§2.18) is held: an attempt that is relaunched sends no `run.failed`, because the run has not
failed, and `run.finished` / `run.failed` go out once, from the last attempt.
...
`attempts` reopen, the only kind that existed. **`as_is` was added 2026-09-13 (#279)**: the same reopen, to be settled
from the story's BRANCH AS IT STANDS with no developer spawned — the review ledger reads it as the flag for the next
Build turn, and it is closed by that turn's `task.done`, so one signature buys one settlement attempt. A fix round opens on that event and closes when the story is `done`
again; one story may have exactly one open at a time. **A Build turn on a reopened story — fix round or plain reopen — whose
developer leaves the tree exactly as it was handed is refused BEFORE its DoD (#308)**: the story blocks (`task.done`,
`status: blocked`), the reason carries the note's first line, no DoD runs, no reviewer is spawned, and the fix round stays open. It is also the one event in the enum that is a **reset
...
**A `check: "dod"` result carries `scope` — and, on a scoped row, `paths` (2026-09-12, #257).** Present ONLY when
the repo declares a `<slot>_scoped` template (§2.1); absent on every other record, which is every record written
before the suffix existed, and every one of those was a full run. `scope: "paths"` ⇒ the template ran over `paths`,
and `command` is the RENDERED line — the template with the paths in place, a path carrying whitespace quoted — which
is not a declared command and not citable: the story's `evidence:` and the handoff's ledger cite the DECLARED
command, and their prose names what ran. `scope: "full"` ⇒ the declared command ran whole. On the epic-head run the
...
| ` ```dod ` block | fenced, ≥1 command | y | Each line must equal a `workspace.yml` command **verbatim**; `dod-gate` re-runs all of them from `repo` and every one must exit `0`. A command the gate REFUSES — not byte-equal to a declared one, or needing a shell — never runs, so it can never be green: it is recorded as REFUSED with the gate's own sentence and **no exit code at all**, in the event, the handoff, the review log and the retro alike (§2.8, §2.9). Editing `workspace.yml` therefore orphans every approved story that cited the old string — `tldrx plan sync-dod` is the mechanical repair, and the drift message names it. A `test_fast:` command is the one declared command a dod block may **not** name: it is the developer's iteration instrument, not the proof of a story, and the refusal names the slot so the reader is not sent looking for a command that is plainly in `workspace.yml` (§2.1). A `<slot>_scoped:` template is the other declared entry a dod block may **not** name: the dod names the FULL command, and the gate narrows it to the story's paths itself, then runs the full command once on the epic head (§2.1, #257). A MUTATING GENERATOR command — a migration-add, a scaffold, an un-verified format, detected BY NAME (`isGeneratorCommand`, `schemas/commandAllowlist.ts`) rather than a declared slot key, since no reserved key names one — is the third declared thing a dod block may **not** name (gh #362): the base-tree pre-flight measures every dod command against the untouched base tree before any story is dispatched, and `dod-gate` re-runs it again at `status: done`, and a generator run a second time either collides with what its first run wrote or silently duplicates it — refused at Plan time, and refused again, as defense in depth, if it ever reaches the base-tree pre-flight (`dodRunner.ts`'s `measureBaseCommand`) some other way. Run a generator once, as the developer; name a build or test that proves what it generated in the dod block instead. |
...
reminder cannot offer two different taps for the same run, and on `gate.requested` that same
branch is NAMED in `detail.holding` (#243) so a consumer routes on a field instead of sniffing a
prefix off the command string; `approve_command` and `reject_command`
are in the `detail` of both either way. `stage` is `<phase>/<stage>` or null. `detail` is per-kind and always an object.

**Kind enum** (closed): `question.raised` `question.timeout` `question.auto_answered` `gate.requested` `gate.timeout`
...
| `question.auto_answered` | the loop took a question's own `Recommended:` option under `questions_policy: recommended` (§2.2, gh #251) — the one kind that asks for NOTHING | `question` (`id`, `title`, `file`), `pick` as `{letter, text}`, `alternatives[]` (the options not taken, as `<letter>) <text>`), `why` (the line's own reason, `""` when it carried none), `fact` (the id written), `decided_by: agent-default`, and `supersede_command`; the top-level `command` is that same `tldrx answer <Qid> "…" --run <id> --supersede` line — the one thing a person can do about it. No `question.raised` is sent for a question the loop answered |
| `gate.requested` | a stage finished and a person must sign it | `cost_usd`, `approve_command`, `reject_command`, `gate_policy` (the stage's frozen `human`/`agent`/`auto`, absent when the run could not be read), and ONE of `held_by` / `signer_held` — the reason the gate is open, from whichever mechanism measured it: the auto verdict carried on the `gate.requested` EVENT for an `auto` policy, the engine signer's `agent.result` for an `agent` one. Absent when nothing looked, which is not the same as nothing found. For a BUILD stage, also `stories` (the `{total, done, in_progress, review, blocked, todo}` counts §5 describes) and, when something is blocked, `blocked_story` + `blocked_reason` — absent on every other stage (#210) — and, when a `todo` story waits on a dependency at `review`/`in_progress` (#280), `waiting_story` + `waiting_on` + `waiting_reason` (the handoff's `## Unknowns` sentence for it), absent when nothing waits (#303). Always `holding` — `questions` \| `stories` \| `none`, the branch `command` was chosen by, said as data (#243). And, ONLY where the gate can name what has to change, `continue_command` (`tldrx reject --run <id> --and-continue --note "…"`, the `…` being the substitutable hole `answer_command` already uses) with `continue_note`, the note derived from `blocked_story` + `blocked_reason`. Both keys absent together on a gate held by questions, on one held by nothing mechanical, and on a `stories` gate whose blocked story recorded no reason: `--note` becomes the next turn's prompt, so a canned one would satisfy the flag and empty the rule it exists for (§7) |
...
The identical hole existed at a GATE and cost the same evening: a run waiting on a signature
got `waiting_on: []` and the same "nothing is waiting on you". A pending gate is now named in
`waiting_on_gate` — a SIBLING key, not a member of `waiting_on`, because an adapter maps every
id in `waiting_on` to `tldrx answer <id>` and a stage id there would make it build a command
nobody can type — the summary says the run is waiting for a person to sign that stage, and
`command` is the literal approve line. Both keys are absent when no gate is pending, so a
heartbeat over a moving run is byte-identical to what it always was. Which gate it is comes
from `waitingFor`, the one derivation `tldrx run status` and the dashboard already share.
...
once, and exits `4` unchanged when it lapses. `--wait-gates <duration>` is its sibling for the
other half of exit `4`: a stage parked on a pending gate is polled for that long — approved,
the loop carries on; rejected, it stops and prints the note — unless the rejection was
`tldrx reject --and-continue`, which asks for another round and the loop re-runs the stage with
the note instead; lapsed, exit `4` with the same lines, after one `gate.timeout`. It WAITS FOR
a signature and never produces one: there is no
engine-side signing in this loop, so a stage on `gates_policy: agent` is waited on exactly as a
`human` one is, until an agent signs it over an evidence note or a person approves it. Nothing
is spent while either flag polls, and the two may be given together.
...
| `tldrx install --claude [--project\|--user] [--skill-only] [--no-hooks] [--no-statusline] [--force-statusline] [--uninstall] [--dry-run]` | `plugin/skills/tldrx/SKILL.md`, `plugin/skills/tldrx-plan/SKILL.md`, the target `.claude/settings.json` | `.claude/skills/tldrx/SKILL.md` and `.claude/skills/tldrx-plan/SKILL.md` (each marked `<!-- tldrx-managed -->`; either one present without the marker refuses the whole install), `.claude/settings.json` (the §4 hooks as `tldrx hook <name>` + `statusLine`), `settings.json.bak-tldrx-<ts>` | 0,1 |
...
| `tldrx run new [--from <path>\|--seed <path> ...] [--scope <s>] [--budget <usd>] [--gates <a,b\|a:agent\|all\|none>] [--questions <a,b\|a:recommended\|all\|none>] [--ship <push\|pr\|merge>] [--attended-by host]` | `workflows/<s>.yml`, `workspace.yml`, `facts.yml`, the `--from` source (§6) or the `--seed` documents (§6.1) | `tldrx-work/<run>/{run.yml,budget.yml,events.jsonl,01-what/*}` incl. the resolved `gates_policy`; `--seed` also writes `01-what/seed-index.md` and declares the documents as What inputs. **`--seed` is repeatable** (§6.2): every occurrence is collected, merged, deduped and re-sorted, and the §6.1 caps apply to the merged set; one occurrence behaves exactly as before. A seed over the threshold or over 10 files adds one **stderr** note naming `tldrx seed triage`. `--gates` LISTS THE HUMAN GATES (`all` = every stage human, `none` = every stage auto); an entry may be QUALIFIED as `<stage>:<policy>` (`plan:agent`) and a bare entry still means `human`, so every existing invocation means what it meant; an unknown stage or an unknown policy is a usage error and no run is created. `--attended-by host` freezes §2.2's `attended_by` into the run: the framework will not spawn on it. Any other value is a usage error and no run is created. `--questions` (gh #251) is `--gates`' grammar for §2.2's `questions_policy`: the list is the stages a PERSON answers, a bare entry means `human`, a qualified one (`plan:recommended`) names the policy, `all` is every stage human and `none` every stage `recommended`; absent ⇒ no `questions_policy` key at all, and the same refusals — unknown stage, unknown policy, empty value — are usage errors with no run created. `--ship <push\|pr\|merge>` freezes §2.2's `ship:` block (gh #253) — how far past the last gate the framework may carry the epic; any other value is a usage error and no run is created; absent ⇒ no block, nothing pushed, nothing opened | 0,1 |
| `tldrx seed check <file\|dir> [--scope <s>] [--budget <usd>]` | the seed (§6.1 rules — the same `collectSeeds`), `workspace.yml` (`commands:`, repos), `workflows/<s>.yml` for the stage split | **nothing** — read-only. stdout: one `file:line rule — text` line per finding, `advisory:` lines that never move the exit code, and with `--budget` the stage split `run new` would write plus the per-story caps (`planBudget`, `caps.ts`). Never a run (#291) | 0,1,3 |
...
| `tldrx run questions set <stage>:<human\|recommended> --note <text> [<run>]` | `run.yml` | `run.yml`'s §2.2 `questions_policy` and `events.jsonl` (`questions.policy_changed`, carrying actor, moment, note and old→new). `run gates set`'s engine and every one of its refusals (gh #251): ONE `<stage>:<policy>` per invocation, the policy named outright, `--note` required, a no-op refused, the FULL map written on a run that had none. It changes who ANSWERS from now on: questions already answered are untouched, and a question the run is parked on now is answered on `run auto`'s next look at it, not by this command — it writes nothing into any questions file | 0,1,2,3 |
| `tldrx run auto [<run>] [--max-usd <n>] [--until <stage>] [--model <m>] [--effort <l>] [--parallel <n>] [--gate-agent] [--notify-every <duration>] [--wait-answers <duration>] [--wait-gates <duration>] [--retry-failed <n>] [--until-done [<n>]] [--rebalance-finished \| --no-rebalance-finished] [--yolo]` | everything `next` reads, once per stage, plus `.tldrx/workspace.yml`'s `notify:` block (§2.18) | everything `next` writes. Refused with **exit 1** on a run marked `attended_by: host`, before the event log is opened so nothing is written: this loop's whole job is calling `next` headless, and on such a run that is a refusal. `--wait-gates <duration>` does the same for a pending gate, resuming on an approval, stopping with the note on a rejection — unless that rejection was `tldrx reject --and-continue`, which re-runs the stage with the note instead — and exiting 4 after one `gate.timeout` when it lapses — it waits for a signature and never produces one. One exception, bounded (gh #231): when an `auto`-policy gate is refused by failed checks ALONE — condition 1 (a declared check failed) and/or condition 5 (`claim-sources` FAILED, not merely unverified) — the loop writes that rejection itself, through the same `reject` path, signed `run auto` (never a person's name, never `auto`), with `and_continue: true` and the failed checks' findings as the note, and re-runs the stage; at most `AUTO_GATE_CHECK_RETRIES` (1) such re-runs per stage, counted off `events.jsonl` over the whole run, after which it waits for a person and prints why. Never on a `human` or `agent` gate, and never when the refusal also names `questions`, `budget`, `stories` or `status`, which have their own paths (a `boundary` with paths outside the surface only warns an `auto` gate since gh #331 and does not block the re-run — unless one of them is in a sensitive class, which still holds it, §5); when a failed check is part of a refusal it does not re-run, the reason is printed once. `--gate-agent` is RENDERING ONLY (§5, "Decision cards"): when the loop stops for a person at exit 4 it prints a decision card in place of the ordinary stop block, and it never upgrades a stage's frozen `gates_policy`. With a `notify:` block declared it also INVOKES that command (§2.18) at each moment a person is needed, writing one `notify.sent` or `notify.failed` per invocation — a failing notifier never changes the exit code. `--notify-every` adds a periodic `status` payload; `--wait-answers` polls the question files instead of exiting 4 at once, and exits 4 unchanged when it lapses. Both refuse a value that is not a duration with exit 1. `--retry-failed <n>` (0..3, refused by name outside that range with exit 1) lets the loop run a FAILED stage again, at most n times IN A ROW, counting only consecutive exit 5s and resetting on any other outcome; it never retries 1, 2 or 3, it spends a fresh metered stage under the same ceilings, and when the bound is spent it stops on the original exit 5 with the count in its last line. `--until-done [<n>]` (gh #252; bare is 5, `0..5`, refused by name outside that with exit 1) is the bound OUTSIDE the loop: the same process relaunches the loop after an exit it can do nothing else with — 5 past the retry bound, 1, 2 with no `budget.blocked` behind it — at most n times, one `run.relaunched` (§2.9) per relaunch, `run.finished` / `run.failed` once from the last attempt; never over 4, never over a `budget.blocked` or its own `--max-usd` (which spans every relaunch), never twice over the same refusal (§ `run.relaunched`). `--rebalance-finished` (gh #314; ON BY DEFAULT since gh #330 — `--no-rebalance-finished` turns it off, both together is exit 1, and `tldrx next` alone never rebalances) lets the budget gate, before it refuses, move exactly the phase's shortfall out of FINISHED phases' unspent ceiling through `budget raise --take-from`'s validation — never the run ceiling, never past a recorded grant (under either `on_grant_exceed`), never a partial move — one `budget.raised` per donor; a shortfall it cannot cover is the same `budget.blocked`, exit 2. **The last mile** (gh #253): the iteration that finds the run `done` on a run whose §2.2 `ship:` block has no `shipped_at` runs `tldrx ship` — the same verb, the same code — and its lines are the loop's; a ship refused is the loop's exit 2, the run's own work staying `done` on disk; a run closed by a hand `tldrx approve` outside the loop is shipped by typing `tldrx ship`. Under §2.2's `questions_policy: recommended` for the CURSOR stage (gh #251) the loop ANSWERS a parked question itself, before any `question.raised` and before any wait: each blocking block whose own `Recommended:` line names one of its options is answered through `tldrx answer`'s path with the option's text, the fact carries `decided_by: agent-default`, `alternatives` and `recommended_why` (§2.5), and one `question.auto_answered` (§2.18) goes out per answer; a block with no recommendation, a recommendation naming no option, or one tagged `irreversible: true` / `money: true` (§2.7) is escalated exactly as under `human`, and when every blocking question is answered the loop resumes on the spot | 0,1,2,3,4,5 |
| `tldrx answer <Qid> <text> [--supersede] [--decided-by <owner\|driver\|agent-default>] [--repo <name>]… [--run <id>]` | `questions.md`, `facts.yml`, `workspace.yml` (for the declared repo names) | `questions.md`, `facts.yml`, `events.jsonl`. `--supersede` is the only writer of `superseded_by`: valid only on an **answered** question, it appends a fact carrying the new answer, sets the old fact's `superseded_by`, appends a superseding `[Answer …]:` line plus a footer to the block, and appends `fact.added` + `fact.superseded`. Without it an answered question is refused (3); with it an **open** one is refused (1). **Both paths** then stamp the earlier-phase documents the block names and append `doc.superseded` (§2.7, *Superseding an earlier phase's document*). `--decided-by` records §2.5's `source.decided_by` and is OPTIONAL here (`facts add` requires it — this path is also the `answer-capture` hook's, which cannot say who answered); absent means "not stated", and stdout says so with the reason. `--repo` is repeatable and scopes the fact's `repos`; without it the scope comes from the question's own `affects:` (§2.7), and from nothing otherwise. Both flags apply to the question the invocation NAMED and to no other block the same call sweeps. New refusals, both **1** and both before anything is written: a `--decided-by` outside `owner\|driver\|agent-default` (§2.5), and a `--repo` no `workspace.yml` repo answers to. A detected contradiction (§2.5) is RAISED, never refused: one extra §2.7 block, one `fact.conflict_raised`, exit unchanged | 0,1,2,3 |
| `tldrx facts add "<text>" --area <id> --decided-by <owner\|driver> [--kind <k>] [--confidence <c>] [--repo <r>] [--run <id>]` | `facts.yml`; `workspace.yml` (for the declared repo names); the open runs, only to establish which one to attribute the fact to | `facts.yml` (one appended row, through `FactsStore` under the workspace lock — load, mint the id, cap, validate, save) and, when a run was established, that run's `events.jsonl` (`fact.added`). The direct writer beside `answer`, for a fact no question asked for. `--area` and `--decided-by` are both REQUIRED: without an area no `{{facts}}` block or no-re-ask hook can scope a match, and without a decider a row that gets cited later cannot say which of the two it was. Over the §2.5 cap the text is cut, marked `truncated: true`, and the cut is named on stdout — a marker only a later reader sees is one the author never acts on. The run is provenance and is ABSENT WITH A REASON when it cannot be established: one open run is used, several are never guessed between, and either way the row is written and stdout says which happened. `--run <id>` naming a run that does not exist is REFUSED (exit 3) before the store is opened — `RunStore.resolve` answers `none` both to “no run is open” and to “that id is not here”, and writing the second as the first attributed the fact to nothing while telling the operator there was nothing to attribute it to. `--repo` is repeatable, scopes the fact's `repos`, de-duplicates (`--repo api --repo api` scopes once), and a name no `workspace.yml` repo answers to is REFUSED (exit **1**) before the store is opened — the same check and the same sentence `tldrx answer --repo` refuses on, one implementation for both, because a fact scoped to a repo that does not exist is never refused later: it is invisible to every `{{facts}}` filter keyed on the real name, forever, with nothing saying why. **`append` dedupes at write time (gh #216):** when the text NORMALISES (trimmed, whitespace collapsed, case-folded — `normaliseFactText.ts`, one derivation) the same as a LIVE fact already on record, nothing is appended and the existing fact's id is printed with "is already on record" instead — a retired or superseded twin does not block, since a fact the workspace no longer stands behind is not "on record" for this purpose | 0,1,3 |
| `tldrx facts dedupe [--dry-run]` | `facts.yml` | `facts.yml` (gh #216 part B) — retires the duplicates that predate the append-time check above: groups LIVE facts by the same normalised text, keeps the EARLIEST id per group, and links every later member `superseded_by` the one before it in the group through `FactsStore.retireDuplicate` (a group of three or more CHAINS rather than fanning every member onto the earliest id directly, because `validateFactsFile`'s reciprocal `supersedes`/`superseded_by` link is one-to-one; `headOf` still resolves any chained member to the one still live). Nothing is deleted (§7: version 1 only grows). `--dry-run` prints the same lines and writes nothing — the file is byte-identical after. No duplicates found ⇒ byte-identical, exit 0 | 0 |
...
| `tldrx reject [--run <id>] --note <text> [--and-continue] [--stage <phase>/<stage>]` | `run.yml` | `run.yml` gate, `events.jsonl`, stage status ⇒ `ready`. `--and-continue` changes nothing this verb does: it records `gate.and_continue: true` (and the same key on the `gate.rejected` event) so an unattended `run auto --wait-gates` re-runs the stage instead of stopping. Without it a rejection stops that loop, which is the default and always was. Refused with **exit 1** beside `--stage`, which is a revoke: that leaves the gate `pending` for a decision nobody has made yet, so there is no rejection for the flag to describe. With `--stage` it REVOKES an approval already given (§5): `gate.revoked`, the cursor moves back, later stages that had run are marked `stale: true`, nothing is deleted. `--stage` may target a FINISHED run | 0,1,2,3 |
...
| `tldrx story reopen <id> [--run <id>] --note <text> [--for-fix \| --as-is]` | `03-plan/waves.yml` + `03-plan/stories/<id>.md` (or `04-build/implicit-plan.yml`), `events.jsonl` | that story file's `status:` ⇒ `todo`, `events.jsonl` (`story.reopened`); reopening a `blocked` story also releases its dependents held ONLY by it, each with its own `story.reopened` carrying `released_by` + `dependency` (#312). Nothing else: no agent, no cost, no stage moved, no worktree or branch touched, and no line of the story but `status:`. Refuses (2) an unknown story id, a `done` story (that is `reject --stage`, or `--for-fix`), a `todo` story, and a missing `--note`. Refuses (1, usage — #227) on a run whose own status is `done` or `cancelled` (`isFinished`), or whose Build gate is signed, regardless of the STORY's own status: a `blocked` story can outlive the run when the Build stage closed with it still unbuilt, or the run was cancelled, with it still unbuilt (`rollUp` derives `done` from the stage, not from every story, and `run cancel` never touches a story file either), and writing `status: todo` there leaves it with nothing that will ever dispatch it — `tldrx reject --stage 04-build/build --note "…"` revokes the gate and reopens the stage, or a new run. With `--for-fix` it opens a FIX ROUND on a `done` story instead (`reason: fix`, no attempt consumed, same DoD + reviewer; a Build turn whose developer leaves the tree as it was handed blocks the story before the DoD, with the note in the reason — #308, and the same for a plain reopen), refusing (2) a story that is NOT done, a missing `--note`, and a story that already has a fix round open. With `--as-is` it reopens the story to be settled from its BRANCH AS IT STANDS (`reason: as_is`): the next Build turn runs the DoD, the review and the merge with NO developer spawned, and blocks the story when the branch carries no commit its epic has not already got or the DoD goes red — except the named review-only case (#295): work already on the epic whose review never completed is settled with nothing merged (`as_is_reason: review-only`). `--for-fix` and `--as-is` together are a usage error (1) | 0,1,2,3 |
...
| `tldrx ship [<run>] [--branch <name>] [--repo <name>] [--base <branch>] [--draft] [--dry-run] [--run <id>]` | `run.yml` (`build.epic_branch`, `ship`), `workspace.yml` (`default_branch`), the run's LAST phase handoff, `04-build/fixlist/*.md`, `03-plan/stories/*.md` (for `touches:`, `status:` and the ```dod block), `workspace.yml`'s `commands:` (the allowlist, and `install:`), and `git`/`gh` for the branch, the remote, the fetched base and any PR already open | **On a run with no `ship:` block, nothing in the workspace** — no event, no gate, no cursor, no `run.yml` field. Outside it: one `gh pr create` per repo the branch is in, with a body RENDERED for a PR (§2.8's handoff inside a `<details>` block) and handed over as `--body-file`, never as an argument. `--dry-run` prints the exact `gh` command and creates nothing, keeping the staged body file so the printed line stays runnable. It pushes ONLY under §2.2's `ship.push` (gh #253) — `git push -u <remote> <branch>`, plain and never forced, through the one push wrapper in the codebase, BEFORE the PR opens, and a push git rejects is a refusal (2) quoting git with no PR opened; under `ship.pr: false` it stops there; under `ship.auto_merge: checks` it asks `gh pr view --json statusCheckRollup` (polled up to 90 s, since a fresh PR reports nothing for its first seconds) and arms `gh pr merge <url> --auto --merge` only over a PR that reports at least one check AND a base branch SEEN to require one (gh #274) — `--auto` waits on the base's requirements, not on the PR's reported checks, so over a base requiring nothing it merges at once. That requirement is read from `gh api repos/{owner}/{repo}/rules/branches/<base>` (rulesets, repository and organization level — GitHub does not return rules whose ruleset is `evaluate` or `disabled`, and a rule that nevertheless arrives carrying an `enforcement` other than `active` is not counted) and, when that finds none, `gh api repos/{owner}/{repo}/branches/<base>/protection` (classic protection). Only a 404 whose body is `Branch not protected` is the answer "none" — any other 404, a branch or repo that is not there among them, is `could not tell`, never "requires nothing". An empty rollup leaves the PR open with `merge: absent — no checks to wait on`; a base requiring nothing with `absent — the base branch requires no check before merge`; a probe that fails or answers non-JSON with `absent — could not tell what the base branch requires`. A requirement imposed by a mechanism neither probe can see reads as "requires nothing" and therefore arms NOTHING — the gap only ever costs a merge that was not armed. On such a run it writes ONE `run.yml` field, the §2.2 `ship:` record; re-run, it asks each repo for an open PR first (one repo included), re-arms a repo whose record says `failed`, leaves one saying `queued` alone, and unions the record. **It REFUSES (2) an epic branch carrying `tldrx-work/`/`.tldrx/` changes**, minus the paths a story at `status: done` declared in its `touches:` (§2.13), and its remedy names only what is still refused. **It REFUSES (2) an epic carrying a story the reviewer REJECTED** (gh #282): a story not at `done` whose last merge into the epic — the last `task.done` for it carrying `commit` and `epic_base` — settled under `changes`. A story merges into its epic BEFORE its review (§5), so that verdict leaves the diff on the branch, and the refusal names the story, that merge and the handoff's reason; a later approved merge over it, a diff that never merged and a `fixlist` signature are not rejections. **It REFUSES (2) an epic carrying a merged diff NOBODY judged** (gh #311): the same read, one predicate apart — a story not at `done` whose last merge settled under a verdict that judged nothing, `n-a` (no reviewer was spawned: the run was cancelled before the review, gh #305, or the turn was refused for want of money, gh #289) or `error` (a reviewer that died mid-read). The refusal names the story, that merge, WHICH of the two it was and the handoff's reason, and its remedy is the review rather than a reopen — a story parked at `review` is settled by `tldrx next` re-running the REVIEW, with no developer spawned. Every verdict a reviewer returned — `approve`, `changes`, `fixlist` — is an opinion and is not this case. **It brings the epic up to date with its base and runs the gate BEFORE anything is pushed or opened** (gh #315): per repo it runs `git fetch <remote> +refs/heads/<base>:refs/remotes/<remote>/<base>` and `git merge-base --is-ancestor` against that ref (a fetch that fails, or an ancestry git cannot answer, REFUSES (2) — "is the epic current" unanswered is not "yes"); then, in a DETACHED worktree of the epic tip in a fresh temp dir (removed on every path), it merges the fetched base with `--no-ff` when the epic is behind — a conflict aborts it and REFUSES (2) naming the paths — and runs the repo's `install:` if declared, then the deduped ```dod commands of the stories settled `done` in that repo, through the same byte-equality allowlist and no-shell split every workspace command obeys, each capped at `SHIP_GATE_TIMEOUT_MS` (1800 s). The first non-zero exit REFUSES (2) naming the command, its exit and its last output line; a command the allowlist or the argv split declines is a refusal too. Green and behind: under `ship.push` the branch is moved FORWARD to the merge — `merge --ff-only` in the checkout that has it out, else `update-ref <new> <old>` — and the push publishes it; never a rebase, never a force. Without `ship.push` the branch is not written to: the gate ran on the merge and the output says the branch is behind. A repo with no `done` story declaring a dod runs nothing and says so. The output gains a `gate:` line per repo. `--dry-run` runs the fetch, the merge and the gate identically and exits with the same code; it skips only the ref move, the push and `gh`. No epic branch, no handoff, no remote, no `gh`, an unpushed branch (without `ship.push`) and an ambiguous branch choice are refusals; a repo whose PR is already open is skipped, so re-running opens nothing twice | 0,1,2,3 |
...
`tldrx dashboard` serves by default; `--serve` says so explicitly and `--static` is the other mode — both together is a refusal (2), never a silent pick. The live page listens on `GET /events` for exactly two Server-Sent Events: `reload`, pushed when a rebuild of the model differs from the one the page is showing, carrying `{at, appended, runs, added, removed}`; and `age`, a bare timestamp every 25 s so the page's ages and its `quiet` mark advance while nothing is happening. A write inside the watched tree that changes nothing the model contains — `.tldrx/cache/`, a static export — pushes neither.
...
| `budget-gate` | PreToolUse (`Bash`) | `tool_input.command` matching `^(claude -p\|tldrx next\|tldrx run auto\|tldrx expert train\|tldrx seed triage)` | `spent + estimate > phase ceiling` (or run ceiling) and `on_exceed: block`. Estimate: the **remaining work** for `next` (below); `--max-usd` else the same figure for `run auto`; $2.00 light / $3.00 `--mode full` for `train` (#96), $1.00 for `triage`. **Except** (gh #321): a `run auto` without `--no-rebalance-finished`, over a PHASE ceiling, whose shortfall finished phases cover whole under the loop's own rebalance rules (`planRebalance`, `--take-from` validation, no grant passed) is allowed with a stderr line and no event — the loop makes the move | Denies the spawn; appends `budget.blocked` |
...
`stories_total`. Since gh #314 it also carries `short_usd`, `finished_unspent_usd` (Σ unspent ceiling over FINISHED phases —
every stage `done`/`skipped`, none stale, `metered-usd`, no unmetered row), `uncovered_usd` and `rebalance_finished`, and the
...
Both counters are recorded on `agent.result` beside `input_tokens` and `output_tokens` — on EVERY turn, including
the Build and Watch turns whose `agent.result` carried no `usage` at all until this was fixed — and, when positive,
on the turn's `run.yml` row as well, so the claim above stays a measurement rather than an argument. Note the ratio while reading them: 40,715 prompt bytes billed as 37,059 cached
...
**The Build developer's preamble is the one exception, role-aware rather than a second copy (gh #364).** Every other
`## Inputs` caller — Watch, the `What` stage — is handed the whole of what it was ever going to see, so "the only
ones you may read" was never false for it. The Build developer works inside a full repo checkout with
`Read`/`Grep`/`Glob`, and it is not: `touches` (and the rest of `## Inputs`) is a WRITE allowlist, never a read one,
and `preamble(inputs, "developer")` says so — measured live, 2026-09-16: a story with 2 `touches` files, both
inlined, read the generic sentence literally, asked whether it could open a port interface its acceptance criteria
needed instead of just reading it, and produced no diff. `## Investigate` step 1 carries the same correction. When
the run's `questions_policy` (§2.2) resolves the Build stage to anything but `human`, the developer prompt also
carries one more rule after step 5: "This run is unattended: nobody answers questions. Decide, state the assumption
in your handoff, and proceed" — and a turn that asks anyway (`questions_asked` non-empty) and leaves no diff earns
`tasks[].failure_kind: "asked_no_diff"` (above) and is requeued rather than left `blocked` for a human to notice.
...
`docker-compose*`/`compose*.yml`, `terraform/`, `.tf`/`.tfvars`/`.hcl`, `infra/`, `k8s/`, `kubernetes/`, `helm/`, `deploy/`,
...
handoff's `## Findings` bullet verbatim, and `waiting_story`, `waiting_on` and `waiting_reason`
for the FIRST `todo` story waiting on a dependency at `review`/`in_progress` — decided by the
Build loop's own `decidingHold`, with the reason being `dependencyWaitReason`, the sentence
`## Unknowns` records (#303). A waiting story was invisible there before: it is not `blocked`
(#280), and `story reopen` refuses a `todo`, so the dependency is the one thing to act on. All of these keys are **additive and present only for a
...
`blocked` story whose log records a dependency hold naming the reopened story — or a story released this way,
transitively — when no other dependency still holds it (`releasedByReopen` in `build/dependencyHold.ts`, asking
`decidingHold`: a dependency at `review`/`in_progress` is a wait and does not keep it). Each release is its own
`story.reopened` (`reason: attempts`, signed by the same actor) with two additive keys, `released_by` (the reopened
story) and `dependency` (the hold its log named), and its note spells the cause. No attempt is consumed. A dependent
also held by another dependency that will not land, or blocked by a verdict rather than a hold, stays `blocked`, and
the output names it with the reason. Everything is prepared and validated before anything is written.

**Settling a story a person finished — `--as-is` (#279).** `tldrx story reopen <id> --as-is --note "…"` reopens the
story to be settled from its BRANCH AS IT STANDS: the next Build turn runs the Definition of Done, the review and the
merge over the commits already on `story/<run>/<id>`, and spawns **no developer**. It exists because the plain verb
cannot reach a story a PERSON finished — a hand rebase after a merge conflict, a fix typed on the branch — since a
developer handed finished work has nothing to commit, and the rule that measures work SINCE THE SPAWN then blocks the
story after one attempt (correctly: work older than the spawn cannot be told apart from a developer that did nothing).
Measured on a real unattended run at 0.18.2: the only way to merge a hand-finished branch was to invent a commit.
It does not weaken that rule — nothing is spawned, so there is nothing to measure work since — and it is not a
shortcut past a gate. Two refusals are structural and no flag skips either: the branch must carry at least one commit
its epic has not already got (measured with the one commit counter, whose `null` is "could not count", never "did not
move"), and the DoD must go green, judged by the same predicate every other story is judged by; the story blocks with
the reason naming which. The story's worktree is found BY NAME from the branch and reopened when the build had removed
it — the usual case, since a blocked story's tree is pruned and a hand fix is typically made in a scratch worktree that
is then removed. `tldrx next --prepare` refuses an `--as-is` story rather than dispatching a developer bundle for it.
The record says who did the work: `story.reopened` carries `reason: as_is` with the actor and note, `task.done` carries
`as_is`, `as_is_by` and `as_is_note`, and the story's review log opens its developer line with **none** and the branch
it took as it stands — nothing anywhere claims a developer delivered it.

**The review-only case (#295).** Under merge-before-review a story's WORK can be entirely on the epic while its
REVIEW is not — the reviewer died, or the attempt after the merge died on a refusal with no work — and then "tip
ahead of base" measures the wrong thing: there is nothing to merge and something to settle. Measured twice on one
day: the only moves were inventing a commit for `--as-is` to take, which corrupts the measurement, or leaving the
story blocked with its dependents. So `--as-is` over a branch that carries nothing beyond its epic asks the review
ledger one more question before refusing: the last `task.done` that recorded a merge (`commit` + `epic_base`, kept
across reopen boundaries as `lastMerge` — a reopen resets what counts against the story, not what the epic holds),
and whether anything judged it. `n-a` and `error` mean nothing did, and the story is settled **review-only**: the
branch is brought up to the epic with `--ff-only` (it carries nothing of its own, so nothing is lost and nothing is
invented), the DoD runs on the epic head, the reviewer is handed the recorded range, and **nothing is merged** —
`carried` is `null` exactly as a resumed review records it. Every other verdict STANDS: `changes` is a fix owed,
`approve` a story done or blocked on its fix list, `fixlist` a signature — and the refusal names the verdict
(`the last review of that work stands`) beside the #279 sentence rather than buying a third opinion on the same bytes.
The record names the case: `task.done` carries `as_is_reason: review-only`, and the review log's developer line reads
**none** with the review-only marker and the commit it reviewed. `--prepare` refuses an `--as-is` story exactly as
before. #279's refusal is untouched — a branch with no recorded merge, or with a standing review, refuses as it did.
...
**`--retry-failed <n>` — a bounded retry on exit `5` (2026-09-10, gh #233).** How many times in a row the loop may
run a FAILED stage again before it stops. `0` — the default, and every invocation written before this — is one
attempt and then exit `5`. A retry is the same `tldrx next` a person would have typed: the stage is on disk as
`failed`, its reason recorded, and the next attempt's prompt is told what the last one did (§5, failure path). It
bounds exit `5` and NOTHING else — `1`, `2` and `4` are each attempted once however large `n` is, because each is a
decision a person owns, and a phase ceiling that means "a human decides about money" would otherwise become a delay.
Only CONSECUTIVE failures count: any other outcome puts the count back to zero, since what is bounded is "this run is
stuck", not "this run has ever failed". A retry SPENDS — a fresh metered stage under the same phase ceiling and the
same `--max-usd` — and when the bound is spent the loop stops on the failure's own exit `5` with the count as its
last line (`3 consecutive stage failures at …`), which is therefore the sentence the `run.failed` notification
carries. `n` outside `0..3` is refused by name with exit `1`; the cap is one constant (`MAX_RETRY_FAILED` in
`core/facilitator/runAuto.ts`), read by the flag parser rather than repeated.

**`--until-done [<n>]` — a bounded relaunch OVER the loop (2026-09-12, gh #252).** `--retry-failed` bounds one
stage's exit `5`s inside the loop; this bounds the loop ENDING. Measured on a 28 h unattended run at 0.16.1: five
exits the loop could do nothing with — a throw that reached the shell as a bare `1` with nothing on the ledger, a
stage failure past the retry bound, refusals whose remedy was the same command typed again — and each one was a person
reading it and relaunching by hand, 18 h before a story ran. So `runAuto` is now a supervisor over the loop body
(`runAutoOnce`), in the SAME process: the loop holds no state and re-reads run.yml every iteration, which is exactly
what makes "run it again" honest. After an exit `1`, `5`, or `2` with no `budget.blocked` behind it, it runs the loop
again, at most `n` times (bare is `5`, the cap `MAX_UNTIL_DONE`; `0` is the default and one launch), writing one
`run.relaunched` (§2.9) per relaunch with the exit it recovered from, the attempt, the bound and the attempt's last
line. The run id resolved by the first attempt is PINNED for every relaunch, and the first attempt's spend baseline is
inherited by every relaunch, so "spent by this loop" and `--max-usd` describe the one command a person typed — a
relaunch that reset the loop's own ceiling would be a ceiling in name only. One derivation (`relaunchVerdict`)
decides both the relaunch and whether the run-end notification (§2.18) is held: an attempt that is relaunched sends
...
3. **The Definition of Done, re-run by the facilitator** in that worktree, through the same runner `dod-gate` uses. All
   ...
   **A DoD is a DELTA gate, so the base tree is checked first (2026-08-31).** At Build entry — after the dirty-tree and
   ...
   nothing. Every row also carries `tree: checkout` (2026-09-16, gh #363, absent-with-reason) — the base pre-flight
   ...
   **A repo's declared `tool_restore:` runs once in the checkout, before the base pre-flight's first probed command
   ...
   `install:` at all — nothing ran it anywhere automatically, so the base pre-flight's own first command refused
   naming a local tool as missing (`dotnet-ef`), with no evidence beyond that refusal. `tool_restore:` is a second
   reserved `commands:` slot (§2.1) for local per-checkout tool state a package manager's own install does not
...
The story's ceiling is `max(price × scale × story_cap_multiplier, story_cap_floor_usd)`, derived at DISPATCH off the
price as it sits on disk — `scale` being `priceScale`, 1 while the plan's prices sum inside the stage's `budget_usd`
and `stage ÷ Σ prices` when they do not (gh #281: a $16.20 stage over a $114.00 plan capped a story priced $14.00 at
$5.97, not $42, and nothing said so): **attempt 1**, the pass the plan priced, gets the whole of it, and the contingency attempt after it
