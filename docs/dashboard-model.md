...
| `spentUsd` / `ceilingUsd` | number \| null | Dollars. Format them; the model does not. `spentUsd` is METERED spend and is a **lower bound** whenever `unmeteredTasks > 0` — read the three fields below with it. `spentUsd` is `run.yml`'s, re-derived on every save. `ceilingUsd` is **`budget.yml`'s** since #236 (resolved in `replay/loadRun.ts`, which both this page and `tldrx replay` load through). Since #245 it is null, never the run.yml creation mirror, when `budget.yml` is missing or will not parse — the fallback used to pair a live `spentUsd` with a ceiling frozen at `run new`, printing an impossible `$X spent of $Y` (`Y < X`) on any run raised after creation. Model version unchanged — the field is still "the live run ceiling, read from the file that holds it", and a value it cannot read was never that field's meaning to begin with. |
| `ceilingBasis` / `ceilingReason` | `"recorded"` \| `"absent"` / string \| null | **Additive (#245).** Why `ceilingUsd` is what it is. `"recorded"` means `budget.yml` parsed and named a figure, `ceilingReason` then null. `"absent"` means `ceilingUsd` is null because `budget.yml` is missing or would not parse; `ceilingReason` names the file and why, one sentence (§7's "absent-with-reason, never invented"). `render.ts` renders the null as `$?` (same as an unreadable `spentUsd`) and puts `ceilingReason` in a `title` attribute rather than showing a figure this process could not read. |
...
| `outcome` | `{kind, text}` \| null | **What the run DELIVERED** (`run.yml` `outcome:`, #210). `kind` is `delivered` \| `partial` \| `nothing-delivered` \| `n/a` \| `not-recorded`; `text` is the sentence `tldrx run status` prints, from the ONE renderer both screens share — `nothing delivered: 0 of 3 stories; S1 — npm run test exited 127`. **Null while the run is open**, which is not the same absence as a closed run whose `outcome:` was never written: that one is `not-recorded`, with the reason inside `text`. Additive — `modelVersion` does not bump. |
| `budget` | `Budget` \| null | **`budget.yml`** (#85) — null when there is none, and null when the file does not parse. `spentUsd` still comes from the run.yml MIRROR; `ceilingUsd` reads this file too since #236, so the headline and this panel cannot quote different ceilings. Fields: `ceilingUsd`, `perAgentMaxUsd`, `warnAtPct`, `onExceed` (each number/string \| null), `economy` (`"metered-usd"` \| `"host-tokens"` — never null; a file with no key means `metered-usd`), `onHostTokensExceed` (`"warn"` \| `"block"` — never null; absence means `warn`), `ceilingHostTokens` (number \| null — **the ceiling `hostTokens` is judged against**, and the reason a host-attended run's dollar meter could not tell the truth without this file), `authorizedUsd` (number \| null — **what the owner AUTHORIZED for this run** (#170); null means no grant is recorded, never `$0`), `authorizedBy` (string \| null — the live fact id the grant cites), `onGrantExceed` (`"warn"` \| `"block"` — never null; absence means `warn`, resolved here against the constant `budget raise` enforces, so the page cannot disagree with the refusal), and `phases` (`BudgetPhase[]`). |
...
null — this phase's own host-token allowance, #61), `authorizedUsd` (number \|
null — this phase's OWN authorization, #170; **null means the RUN's grant governs
it**, which is a different statement from "the owner authorized nothing here").
