...
| `spentUsd` / `ceilingUsd` | number \| null | Dollars. Format them; the model does not. `spentUsd` is METERED spend and is a **lower bound** whenever `unmeteredTasks > 0` — read the three fields below with it. `spentUsd` is `run.yml`'s, re-derived on every save. `ceilingUsd` is **`budget.yml`'s** since #236 (resolved in `replay/loadRun.ts`, which both this page and `tldrx replay` load through), falling back to `run.yml`'s mirror only when `budget.yml` will not parse: the mirror is the ceiling the run was CREATED with and a `budget raise` never moves it, so the headline used to be able to contradict the `budget` panel on this same page. Model version unchanged — the field is the live run ceiling it always claimed to be, read from the file that holds it. |
