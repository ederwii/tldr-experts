...
**Name decided:** brand/repo `tldr-experts`, CLI `tldrx`; npm package `tldr-experts` installing the `tldrx` command plus a `tldr-experts` alias (2026-08-29: unscoped `tldrx` as a package name is refused by npm's similarity rule — too close to `tsdx`). Framework dir `.tldrx/`, runs in `tldrx-work/`.
