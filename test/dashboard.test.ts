...
describe("the dashboard model", () => {
  ...
  test("its field names are the contract a designer targets", () => {
    expect([...fieldPaths(model)].sort()).toEqual([
      ...
      "runs[].path[].expert",
      "runs[].path[].gate",
      "runs[].path[].gateAuthority",
      "runs[].path[].gateBy",
      "runs[].path[].gateEvidence",
      "runs[].path[].gateExecutedBy",
      "runs[].path[].gateNote",
