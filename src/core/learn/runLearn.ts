...
export function ending(root: string, io: LearnIo): string {
  ...
  return [
    ...
    "      Detection, plus one run of your repo's own build/test/lint/typecheck commands so",
    "      `workspace.yml` records whether they WORK (`--no-probe` skips that, and is the flag",
    "      to use on a repo you have not read). No model, nothing spent. It ends by offering",
