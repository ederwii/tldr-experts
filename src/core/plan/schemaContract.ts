...
export function renderPlanSchemaContract(): string {
  ...
  return [
    ...
    "plus one fenced ```dod block. Every key is on its OWN line: `acceptance: [ … ]` and",
    "`test_plan: [ … ]` are two lines, never one line joined by a comma — a comma between two",
    "keys turns the line into a flow mapping and the whole block stops parsing (gh #288: a live",
    "run wrote all three of its stories that way and paid for a second plan). These keys, in",
    "this order — every one required except",
