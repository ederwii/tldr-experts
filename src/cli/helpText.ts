...
const ENTRIES: readonly CommandHelp[] = [
  ...
  },
  {
    ...
    flags: [
      ...
      },
      {
        name: "until-done",
        arg: "[<n>]",
        meaning: "Relaunch the loop IN-PROCESS after an exit it can do nothing else with, at most n times — bare means 5, the cap. `--retry-failed` bounds one stage's exit 5s INSIDE the loop; this bounds the loop ENDING: a stage failure past that bound (5), a thrown error that used to reach the shell as a bare 1 with nothing on the ledger, a refusal whose remedy is the same command typed again (2). Measured on one 28 h unattended run: five such exits, each a person reading it and typing `tldrx run auto` again, 18 h before a story ran. Every relaunch is a `run.relaunched` event with the exit it recovered from, the attempt and the bound, and `run.finished` / `run.failed` go out ONCE, from the last attempt. What it NEVER relaunches over: exit 4 (a person's — `--wait-answers` and `--wait-gates` own it), a `budget.blocked` or this loop's own `--max-usd` (nothing in-process moves a ceiling past what `--rebalance-finished` already tried before refusing, and `--max-usd` spans every relaunch rather than resetting), and an attempt REFUSED BY THE SAME WORDS as the one before it — a refusal that repeats is not one a relaunch moves. The words compared are the ones the stop NAMES, never the last line printed: every stage death ends with the same advice about cost, and comparing that stopped runs that were making progress (#297). `0` is the default and is one launch, exactly as before. Put the run id before the flag, or write `--until-done=3`.",
        sub: "auto",
      },
      {
        name: "rebalance-finished",
        arg: null,
        meaning: "Before a phase is refused on money (`budget.blocked`), move EXACTLY its shortfall out of FINISHED phases' unspent ceiling — the `tldrx budget raise <phase> <usd> --take-from <finished>` a person would type — and carry on. Finished means every stage of that phase is `done` or `skipped`, none is stale, it is priced in metered-usd, and none of its turns is unmetered (so what it has left is provably unspendable); a phase with a stage still to run — 05-watch while Build is blocked — never gives. Donors give in run order. The run ceiling never grows, a move that would put the phase past a recorded grant is not made (not even under on_grant_exceed: warn), and if all finished phases together cannot cover the shortfall NOTHING moves and the refusal says how short it is. Each move is one `budget.raised` event with before/after for both phases, `take_from`, `source: run auto --rebalance-finished`, and your name as actor. ON BY DEFAULT (gh #330) — this flag only says the default out loud; `--no-rebalance-finished` turns it off. Why the default flipped: an audit of unattended runs counted 22 human interventions on stage/phase sizing and 27 `budget.raised` events (the audit's measured counts), none of whose notes changed the work (the audit's reading, inferred), while this move ran twice in a validation run with nobody present. `tldrx next` alone never rebalances.",
        sub: "auto",
      },
      {
        name: "no-rebalance-finished",
        arg: null,
        meaning: "Turn the default rebalance OFF for this launch: a phase that cannot afford its next stage is refused on money (`budget.blocked`, exit 2) even when finished phases hold enough unspent ceiling, and the refusal names that money and the exact `tldrx budget raise <phase> <usd> --take-from <finished>` a person would type. For a launch whose phase ceilings must mean exactly what they were set to. Refused beside `--rebalance-finished` (exit 1): the two are opposite directions.",
        sub: "auto",
      },
      {
        name: "wait-gates",
    ...
    examples: [
      ...
      "tldrx run auto 260101-checkout --retry-failed 2 --until-done --wait-answers 4h --wait-gates 4h",
    ...
    notes: [
      ...
      "`run auto` is an ENGINE, not a lock. It calls `next` HEADLESS over and over, so THE FRAMEWORK spawns a metered sub-agent stage after stage, and it stops at the first thing it may not decide: a human gate or an open question (4), a stage failure (5) \u2014 unless `--retry-failed <n>` lets it run that stage again, bounded, up to n times in a row \u2014 a phase ceiling or this loop's own --max-usd (2). `--until-done [<n>]` puts a bounded relauncher OVER those stops: after a 5 past the retry bound, a throw, or a 2 with no money behind it, the same process runs the loop again and writes `run.relaunched` \u2014 never over a person's 4, never over a `budget.blocked`, never twice over the same refusal. It is REFUSED ON AN ATTENDED RUN (exit 1, before the event log is opened) \u2014 a lock and an engine are alternatives, never layers.",
      ...
      "`run cancel` closes a run for good: cancelled is terminal, so `tldrx status` and every id-less command stop seeing it. Nothing of the run is deleted — the stages, outputs, events and money spent stay on disk and `tldrx replay <id>` still reads them. It RELEASES the epic branch the run claimed (#272): `epic/<slug>` with no commit beyond the default branch is deleted, one with commits is renamed to `epic/<slug>@<run-id>` so they survive and the name is free for the next run of the same feature; recorded on run.yml (`build.epic_released`) and as `epic.released`. A branch checked out in a worktree is left alone and named; story branches are never touched.",
    ],
  },
  {
    name: "seed",
  ...
  },
  {
    ...
    flags: [
      ...
      },
      {
        name: "check",
        arg: null,
        meaning: "--commit only: REHEARSE the commit. Validates the prepared bundle's result.json through the same reader --commit uses, prints every refusal with the offending line, and WRITES NOTHING \u2014 no state, no event, no attempt spent. Exit 0 when --commit would read the envelope, 1 when it would not. Its reason for existing is the reviewer envelope: a `refuted` finding whose `[src: \u2026]` citation is not the last thing on its line is refused at --commit --review, and until now that refusal could only arrive after the turn had been paid for. On a DEVELOPER bundle the reader is deliberately tolerant \u2014 a missing `outputs` is read as `[]` and a `notes` written as an array of strings is joined with newlines, neither refused \u2014 so --check exits 0 and NAMES what is about to be coerced or joined. It checks the result envelope, not the stage's gate.",
      },
      {
        name: "fixlist",
        arg: "<path>",
  ...
  },
  {
    ...
    flags: [
      ...
      },
      {
        name: "as-is",
        arg: null,
        meaning: "Settle the story from its BRANCH AS IT STANDS: the next Build turn runs the dod, the review and the merge over the commits already on `story/<run>/<id>`, and spawns NO developer. For a story a person finished by hand \u2014 a rebase after a conflict, a fix typed directly on the branch \u2014 which the plain verb cannot land, because a developer handed finished work has nothing to do and the story blocks on the no-work rule. It is not a shortcut past a gate: a red dod blocks the story, and so does a branch carrying no commit its epic has not already got \u2014 with one named exception: a story whose work is ALREADY on the epic from an earlier turn and whose review never completed (the reviewer died, or the attempt after the merge died with no work) is settled review-only: nothing is merged, the dod runs on the epic head and the reviewer judges the range the story was merged as; a review that stands (`approve`, `changes`, `fixlist`) still refuses. Requires --note; refused together with --for-fix.",
        sub: "reopen",
      },
      runFlag(),
      root(),
    ...
    notes: [
      ...
      "Reopening a `blocked` story also releases its dependents that were blocked ONLY by it, in the same command: every story whose recorded block is `dependency <id> blocked` naming it \u2014 or naming a story released this way, transitively \u2014 goes back to `todo`, provided no OTHER dependency still holds it. Each release is its own `story.reopened` signed by the same actor, with `released_by` (the reopened story) and `dependency` (the hold it named), and consumes no attempt. A dependent also held by another dependency that will not land, or blocked by a reviewer, a DoD or a developer rather than a hold, stays `blocked`; the output lists what was released and what stayed, with the reason.",
  ...
  },
  {
    ...
    flags: [
      ...
      { name: "dry-run", arg: null, meaning: "Run every check \u2014 the base fetch, the merge and the gate included \u2014 and print the exact `gh` command, creating nothing, pushing nothing and moving no branch." },
      runFlag(),
      root(),
    ],
    examples: [
    ...
    notes: [
      ...
      "It REFUSES an epic carrying a story the reviewer REJECTED (exit 2 \u2014 a gate said no, #282). A story merges into its epic BEFORE its review, by design, so a `changes` verdict leaves the diff on the branch: with one story `done` beside it the PR opened with the rejected code in its diff and a body that listed the story under \"Not done\" without saying so. The refusal reads the ledger's last merge for every story not `done` \u2014 the last `task.done` carrying `commit` and `epic_base` \u2014 and names the story, that merge and the handoff's own reason. A `changes` a later attempt merged over and got approved is not a rejection, a story whose diff never merged has nothing to refuse, and a story parked at `review` under a SIGNED fix list ships with its findings listed. `--dry-run` is refused in the same words.",
      "It will not open a PR over a STALE or RED epic (exit 2 \u2014 a gate said no, #315). Before anything is pushed or opened it fetches each repo's base from the remote and checks the epic contains it; in a throwaway worktree it merges the base in when the epic is behind, then runs the repo's `install:` and the Definition of Done of every story settled `done` there \u2014 the tree the PR would merge. A conflict refuses naming the paths; a red command refuses naming the command and its exit; a failed fetch refuses. When green and behind, a run with `ship.push` gets the base merged INTO the epic \u2014 a forward merge commit, never a rebase or a force \u2014 and pushed with it; a run without it keeps its branch untouched and is told the branch is behind. Every PR it opens says what the gate ran on a `gate:` line. It runs only what the plan declared: a check that lives only in the remote's CI, a coverage threshold say, is still the CI's to catch.",
